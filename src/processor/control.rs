...
use crate::{
    ...
    processor::{SessionRegistry, upgrade::UpgradeReq},
...
impl StatusCtx {
    fn snapshot(&self) -> WorkerStatus {
        let in_flight = self.registry.lock().unwrap().sessions.len() as u64;
        WorkerStatus {
    ...
    loop {
        ...
        while let Some(line) = reader.next().await {
            ...
            match parsed {
                ...
                Ok(ControlMsg::Drain) => {
                    tracing::debug!("control-plane Drain (processor: noop)");
                }
